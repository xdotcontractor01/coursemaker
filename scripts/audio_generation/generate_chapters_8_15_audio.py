#!/usr/bin/env python3
"""
Audio Generation Script for Chapters 8-15
...
def normalize_audio(input_path: Path, output_path: Path) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"[WARNING] FFmpeg normalization failed: {result.stderr[:100]}")
            return False
...
def main():
    ...
    print("=" * 60)
    print("Generating Audio for Chapters 8-15")
    print("=" * 60)
    
    total_all = 0.0
    for chapter_num in range(8, 16):
        generate_audio_for_chapter(chapter_num, client)
        # Read updated manifest for total
        manifest_path = MANIFESTS_DIR / f"chapter{chapter_num:02d}.json"
        if manifest_path.exists():
            with open(manifest_path, 'r') as f:
                manifest = json.load(f)
                total_all += manifest.get('total_duration', 0) or 0
    
    print("\n" + "=" * 60)
    print(f"Audio generation complete!")
    print(f"Total runtime: {total_all:.1f}s ({total_all/60:.1f} min)")
    print("=" * 60)
