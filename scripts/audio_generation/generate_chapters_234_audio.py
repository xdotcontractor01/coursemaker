#!/usr/bin/env python3
"""
Audio Generation Script for Chapters 2, 3, and 4
    ...
    for scene in manifest['scenes']:
        ...
        try:
            ...
            if normalized:
                ...
                # If normalization failed, use raw file
                temp_path.rename(audio_path)
                print(f"       -> Using unnormalized audio")
            
            # Update manifest
            scene['expected_duration'] = duration
            total_duration += duration
            
        except Exception as e:
            print(f"       -> ERROR: {e}")
            scene['expected_duration'] = 0.0
    
    # Update manifest with total duration
    manifest['total_expected_duration'] = total_duration
    
    # Save updated manifest
    with open(manifest_path, 'w') as f:
        json.dump(manifest, f, indent=2)
    
    print(f"\n  Total: {total_duration:.1f}s ({total_duration/60:.1f} min)")
...
def main():
    ...
    print("=" * 60)
    print("Generating Audio for Chapters 2, 3, and 4")
    print("=" * 60)
    print(f"TTS Model: {TTS_MODEL}")
    print(f"Voice: {TTS_VOICE}")
    print(f"Normalization: -{abs(LUFS_TARGET)} LUFS")
    print("=" * 60)
    
    success = True
    for chapter_num in [2, 3, 4]:
        if not generate_audio_for_chapter(chapter_num, client):
            success = False
    ...
    if success:
        ...
        print("=" * 60)
        print("\nNext steps:")
        print("  1. Run dryrun_chapters_234.py to verify audio files")
        print("  2. Run render scripts to generate videos")
    else:
        print("\n" + "=" * 60)
        print("Audio generation completed with errors")
        print("=" * 60)
        sys.exit(1)
