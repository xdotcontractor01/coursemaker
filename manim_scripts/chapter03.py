...
Pages 13-14 of Basic Highway Plan Reading
...
MANIFEST_PATH = PROJECT_ROOT / "manifests/chapter_03.json"
AUDIO_DIR = PROJECT_ROOT / "audio"
ASSETS_DIR = PROJECT_ROOT / "assets/images/chapter3"
AUDIO_BUFFER = 0.12
...
def load_image_safe(image_path: str, scale: float = IMAGE_SCALE_FACTOR) -> Mobject:
    ...
    try:
        # Extract filename from path like "assets/images/chapter3/figure_3_1.jpg"
...
class Chapter03Video(Scene):
    ...
    def construct(self):
        ...
        print("\n" + "=" * 60)
        print(f"Chapter 3: {manifest['title']}")
        print("=" * 60)
        ...
        self.scene_2_typical_sections(manifest['scenes'][1])
        self.scene_3_required_pavement(manifest['scenes'][2])
        self.scene_4_horizontal_distance(manifest['scenes'][3])
    ...
    def scene_1_title(self, scene_data):
        ...
        # Visual elements
        chapter = Text("Chapter 3", font_size=SUBTITLE_FONT_SIZE, color=MUTED_COLOR)
        title = Text("Typical Sections", font_size=MAIN_TITLE_FONT_SIZE, weight=BOLD, color=TITLE_COLOR)
        subtitle = Text("Cross-Sectional Roadway View", font_size=SUBTITLE_FONT_SIZE, color=BODY_COLOR)
        pages = Text("Pages 13-14", font_size=24, color=MUTED_COLOR)
    ...
    def scene_2_typical_sections(self, scene_data):
        """Explain Typical Sections."""
        audio_path = AUDIO_DIR / scene_data['tts_file_path'].replace('audio/', '')
        ...
        # Visual elements
        heading = create_section_heading("Introduction to Typical Sections")
    ...
    def scene_3_required_pavement(self, scene_data):
        """Explain Required Pavement."""
        audio_path = AUDIO_DIR / scene_data['tts_file_path'].replace('audio/', '')
        duration = scene_data.get('expected_duration', 10.0)
        ...
        # Visual elements
        heading = create_section_heading("Required Pavement")
        bullets = create_bullet_list(scene_data['bullets'])
        bullets.next_to(heading, DOWN, buff=0.8)
        bullets.to_edge(LEFT, buff=CONTENT_LEFT_BUFF + 0.5)
    ...
    def scene_4_horizontal_distance(self, scene_data):
        """Explain Horizontal Distance."""
        audio_path = AUDIO_DIR / scene_data['tts_file_path'].replace('audio/', '')
        duration = scene_data.get('expected_duration', 10.0)
        ...
        # Visual elements
        heading = create_section_heading("Horizontal Distance")
        bullets = create_bullet_list(scene_data['bullets'])
        bullets.next_to(heading, DOWN, buff=0.8)
        bullets.to_edge(LEFT, buff=CONTENT_LEFT_BUFF + 0.5)
...
if __name__ == "__main__":
    print(f"\nTo render: manim -qh manim_chapter03.py Chapter03Video")
