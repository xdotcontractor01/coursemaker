...
Pages 73-76 of Basic Highway Plan Reading
...
MANIFEST_PATH = PROJECT_ROOT / "manifests/chapter12.json"
...
class Chapter12BaseScene(Scene):
    def render_scene(self, scene_data, manifest):
        ...
        if scene_index == 1:
            chapter_text = Text("Chapter 12", font_size=SUBTITLE_FONT_SIZE, color=MUTED_COLOR)
            title_text = Text("ESPCP", font_size=MAIN_TITLE_FONT_SIZE, weight=BOLD, color=TITLE_COLOR)
            subtitle_text = Text("Erosion, Sedimentation & Pollution Control", font_size=28, color=MUTED_COLOR)
...
class Chapter12Full(Chapter12BaseScene):
    """Full Chapter 12 - all scenes"""
    
    def construct(self):
        manifest = load_manifest()
        print("\n" + "=" * 60)
        print(f"Chapter 12: {manifest['title']}")
        print("=" * 60)
