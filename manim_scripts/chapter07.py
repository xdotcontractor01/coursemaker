...
Pages 37-50 of Basic Highway Plan Reading
...
MANIFEST_PATH = PROJECT_ROOT / "manifests/chapter07.json"
...
def load_image_safe(image_path: str, scale: float = IMAGE_SCALE_FACTOR) -> Mobject:
    ...
    try:
        # Handle paths like "assets/images/chapter7/figure_7_1.jpg"
        ...
        if not full_path.exists():
            ...
            if not full_path.exists():
                ...
                if len(parts) >= 3:
                    chapter_dir = parts[-2]  # e.g., "chapter7"
                    filename = parts[-1]  # e.g., "figure_7_1.jpg"
...
class Chapter07BaseScene(Scene):
    ...
    def render_scene(self, scene_data, manifest):
        ...
        if "Title" in title or scene_index == 1:
            # Title scene
            chapter_text = Text("Chapter 7", font_size=SUBTITLE_FONT_SIZE, color=MUTED_COLOR)
...
class Chapter07Lesson01(Chapter07BaseScene):
    """Chapter 7, Lesson 1: Plan View & Horizontal Alignment (scenes 1-5)"""
    ...
    def construct(self):
        ...
        print("\n" + "=" * 60)
        print(f"Chapter 7, Lesson 1: Plan View & Horizontal Alignment")
        print("=" * 60)
        
        # Scenes 1-5
        scenes_to_render = [s for s in manifest['scenes'] if 1 <= s['index'] <= 5]
        
        for scene_data in scenes_to_render:
            self.render_scene(scene_data, manifest)


class Chapter07Lesson02(Chapter07BaseScene):
    """Chapter 7, Lesson 2: Profile View & Vertical Alignment (scenes 6-10)"""
    ...
    def construct(self):
        ...
        print("\n" + "=" * 60)
        print(f"Chapter 7, Lesson 2: Profile View & Vertical Alignment")
        print("=" * 60)
        
        # Scenes 6-10
        scenes_to_render = [s for s in manifest['scenes'] if 6 <= s['index'] <= 10]
        
        for scene_data in scenes_to_render:
            self.render_scene(scene_data, manifest)


class Chapter07Lesson03(Chapter07BaseScene):
    """Chapter 7, Lesson 3: Construction Elements (scenes 11-15)"""
    ...
    def construct(self):
        ...
        print("\n" + "=" * 60)
        print(f"Chapter 7, Lesson 3: Construction Elements")
        print("=" * 60)
        
        # Scenes 11-15
        scenes_to_render = [s for s in manifest['scenes'] if 11 <= s['index'] <= 15]
