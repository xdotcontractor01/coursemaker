...
Pages 77-84 of Basic Highway Plan Reading
...
MANIFEST_PATH = PROJECT_ROOT / "manifests/chapter13.json"
...
class Chapter13BaseScene(Scene):
    def render_scene(self, scene_data, manifest):
        ...
        if scene_index == 1:
            chapter_text = Text("Chapter 13", font_size=SUBTITLE_FONT_SIZE, color=MUTED_COLOR)
            ...
            if image_paths:
                ...
                if len(image_group) > 0:
                    if len(image_group) <= 2:
                        ...
                        image_group.arrange_in_grid(rows=2, buff=0.3)
                    image_group.scale_to_fit_height(4.5)
                    image_group.next_to(bullet_list, RIGHT, buff=0.8)
...
class Chapter13Lesson01(Chapter13BaseScene):
    """Chapter 13, Lesson 1: Cross Sections and Earthwork (scenes 1-4)"""
    
    def construct(self):
        manifest = load_manifest()
        print("\n" + "=" * 60)
        print(f"Chapter 13, Lesson 1: Cross Sections and Earthwork")
        print("=" * 60)
...
class Chapter13Lesson02(Chapter13BaseScene):
    """Chapter 13, Lesson 2: Grade, Slopes, and Slope Stakes (scenes 5-8)"""
    
    def construct(self):
        manifest = load_manifest()
        print("\n" + "=" * 60)
        print(f"Chapter 13, Lesson 2: Grade, Slopes, and Slope Stakes")
        print("=" * 60)
        
        scenes_to_render = [s for s in manifest['scenes'] if 5 <= s['index'] <= 8]
        for scene_data in scenes_to_render:
            self.render_scene(scene_data, manifest)


class Chapter13Full(Chapter13BaseScene):
    """Full Chapter 13 - all scenes"""
    
    def construct(self):
        manifest = load_manifest()
        print("\n" + "=" * 60)
        print(f"Chapter 13: {manifest['title']} (Full)")
        print("=" * 60)
